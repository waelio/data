...
function tmpFile() {
  return path.join(os.tmpdir(), `waelio-data-test-${crypto.randomBytes(6).toString('hex')}.json`);
}
...
describe('Database', () => {
  ...
  describe('encryption', () => {
    it('stores encrypted data and decrypts on read', () => {
      ...
      assert.ok(!raw.includes('hunter2'), 'plaintext should not appear in encrypted file');
    ...
    it('throws on invalid encryption key length', () => {
      assert.throws(() => new Database({ encryptionKey: 'tooshort' }), /32 bytes/);
    });
  });
});
