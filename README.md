...
// Write
await fetch(`${BASE}/users/alice`, { method: 'POST', headers, body: JSON.stringify({ name: 'Alice' }) });

// Read
const { value } = await fetch(`${BASE}/users/alice`, { headers }).then(r => r.json());
...
```js
db.on('change',  ({ event, collection, key, value }) => { /* any mutation */ });
db.on('set',     ({ event, collection, key, value }) => { /* key written  */ });
db.on('delete',  ({ event, collection, key })        => { /* key deleted  */ });
db.on('clear',   ({ event, collection })             => { /* col cleared  */ });
```
